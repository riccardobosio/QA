...
def rerank(all_contexts, num_reranked_contexts, top_n_answers):
    ...
    added_ctx = []
    for index, ctx in enumerate(all_contexts):  # DUBBIO: I CONTESTI SONO ORDINATI
        text = _remove_punctuation(_lowercase(ctx.text)).split()  # I have a list of tokens (strings) here
        for ans in top_n_answers:
            flag = False
            ans_text = _remove_punctuation(_lowercase(ans.text)).split()  # I have a list of tokens here
            for i in range(0, len(text) - len(ans_text) + 1):
                if ans_text == text[i: i + len(ans_text)]:
                    ...
                    break
            # if the context contains each word of one of the top answers, then it added to the ordered contexts
            if flag:
                break
    # add the remaining contexts
